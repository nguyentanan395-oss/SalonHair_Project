...
if (appRoot) {
    const dom = {
        ...
        feedbackShapeButtons: Array.from(document.querySelectorAll(".feedback-shape-button")),
    ...
    async function boot() {
        setStatus("Đang tải mô hình AI lần đầu. Nếu mạng chậm có thể mất 10-15 giây...", "loading");
        ...
        try {
            ...
            setStatus("AI đã sẵn sàng. Bạn có thể mở webcam trực tiếp hoặc tải ảnh chân dung.", "ready");
            dom.startCameraButton.disabled = false;
            updateBatchUi();
            await loadRuntimeModel();
        } catch (error) {
            console.error(error);
            setStatus("Không tải được mô hình AI. Hãy tải lại trang hoặc kiểm tra mạng/CDN.", "error");
        }
    }
    ...
    function bindEvents() {
        ...
        dom.feedbackShapeButtons.forEach((button) => {
            button.addEventListener("click", () => saveFeedback(button.dataset.shape || ""));
        });
        dom.exportFeedbackButton?.addEventListener("click", exportDataset);
        dom.importFeedbackButton?.addEventListener("click", () => dom.importFeedbackInput?.click());
        dom.importFeedbackInput?.addEventListener("change", importDataset);
        dom.trainModelButton?.addEventListener("click", trainModel);
        dom.selectBatchFolderButton?.addEventListener("click", () => dom.batchFolderInput?.click());
    ...
    async function startCamera() {
        if (!state.faceLandmarker) {
            setStatus("Mô hình AI chưa sẵn sàng. Hãy đợi thêm một chút rồi thử lại.", "error");
            return;
        }
        ...
        try {
            ...
            dom.uploadHint.textContent = "Webcam đang chạy realtime. Hãy giữ khuôn mặt chính diện để AI nhận diện ổn định hơn.";
            dom.startCameraButton.disabled = true;
            dom.stopCameraButton.disabled = false;
            dom.analyzeButton.disabled = true;

            await dom.video.play();
            setStatus("Webcam đã mở. AI đang quét khuôn mặt theo thời gian thực...", "ready");
            ...
            setStatus("Không mở được webcam. Hãy kiểm tra quyền camera của trình duyệt.", "error");
        }
    }
    ...
    function stopCamera() {
        ...
        if (!state.image) {
            dom.emptyState.classList.remove("d-none");
            dom.uploadHint.textContent = "Bấm mở webcam để quét trực tiếp, hoặc tải ảnh chân dung một người nếu cần tư vấn thủ công.";
            resetMetrics();
            clearCanvas();
        }
    }
    ...
    async function handleFileSelect(event) {
        ...
        try {
            await loadFileIntoStage(file, {
                hintTextFactory: (image) => `${file.name} | ${image.naturalWidth} x ${image.naturalHeight}`,
    ...
    async function analyzeImage() {
        ...
        try {
            ...
            console.error(error);
            setStatus("Không thể phân tích ảnh này. Hãy thử ảnh sáng và rõ mặt hơn.", "error");
        }
    }
    ...
    async function processVideoFrame() {
        ...
        if (dom.video.currentTime !== state.lastVideoTime) {
            ...
            try {
                const detection = state.faceLandmarker.detectForVideo(dom.video, nowInMs);
                handleDetectionResult(detection, "video");
                state.lastVideoTime = dom.video.currentTime;
            } catch (error) {
                console.error(error);
                setStatus("AI bị gián đoạn khi đọc webcam. Hãy dừng rồi mở lại camera.", "error");
                stopCamera();
                return;
            }
        }
    ...
    async function handleDetectionResult(detection, source) {
        ...
        if (faces.length === 0) {
            ...
            if (source === "video") {
                ...
                setStatus("Không nhận ra khuôn mặt trong ảnh. Hãy chọn ảnh chính diện và sáng hơn.", "error");
            }
        ...
        if (faces.length > 1) {
            clearCanvas(source);
            clearFeedbackState();
            setStatus("AI thấy nhiều hơn một khuôn mặt. Hãy giữ khung chỉ còn một khách.", "error");
            resetMetrics();
            return null;
        }
        ...
        if (source === "video") {
            setStatus("Webcam đang quét ổn định. AI sẽ cập nhật gợi ý khi dáng mặt đủ rõ.", "ready");
        } else {
            setStatus("Phân tích ảnh xong. Đang ghép gợi ý kiểu tóc phù hợp...", "ready");
        }
    ...
    async function maybeSendAnalysis(analysis, metrics, source, analysisToken) {
        const now = Date.now();
        const forceFreshSuggestion = isStillImageSource(source) || dom.results.classList.contains("d-none");
        if (state.isSending) {
            if (forceFreshSuggestion) {
                state.queuedAnalysisRequest = { analysis, metrics, source, analysisToken };
            }
            return;
        }

        const shouldSend =
            forceFreshSuggestion || analysis.shape !== state.lastSentShape || now - state.lastSentAt >= UPDATE_COOLDOWN_MS;
        ...
        try {
            ...
            console.error(error);
            setStatus("AI đọc được khuôn mặt nhưng chưa lấy được gợi ý từ server. Hãy thử lại.", "error");
        } finally {
            state.isSending = false;
            if (state.queuedAnalysisRequest) {
                const queuedRequest = state.queuedAnalysisRequest;
                state.queuedAnalysisRequest = null;
                maybeSendAnalysis(queuedRequest.analysis, queuedRequest.metrics, queuedRequest.source, queuedRequest.analysisToken);
            }
        }
    }
    ...
    async function saveFeedback(correctedShape) {
        ...
        if (!correctedShape) {
            setFeedbackStatus("Bạn cần chọn nhãn đúng trước khi lưu feedback.", "error");
            return;
        }
        ...
        try {
            ...
            setDatasetStatus(`Dataset hiện có ${payload.totalSamples} mẫu.`, "success");

            if (state.batch.active) {
                state.batch.labeledCount += 1;
                state.batch.index += 1;
                shouldAdvanceBatch = true;
                setBatchStatus(`Đã lưu ${state.batch.currentFileName}. Đang mở ảnh tiếp theo...`, "success");
                updateBatchUi();
            }
        } catch (error) {
            console.error(error);
    ...
    async function importDataset(event) {
        ...
        try {
            ...
            if (typeof payload.totalSamples === "number") {
                setFeedbackStatus(`Tổng dataset hiện tại: ${payload.totalSamples} mẫu.`, "success");
            }
        } catch (error) {
            console.error(error);
            setDatasetStatus("Import dataset thất bại. Hãy kiểm tra file zip và thử lại.", "error");
        } finally {
            event.target.value = "";
        }
    }
    ...
    async function trainModel() {
        ...
        try {
            ...
            console.error(error);
            setDatasetStatus("Train model thất bại. Hãy kiểm tra dataset rồi thử lại.", "error");
        } finally {
    ...
    async function handleBatchFolderSelect(event) {
        ...
        if (!state.faceLandmarker) {
            setBatchStatus("Mô hình AI chưa sẵn sàng. Hãy đợi thêm một chút rồi thử lại.", "error");
            return;
        }
        ...
        updateBatchUi();
        setBatchStatus(`Đã nạp ${files.length} ảnh. Đang mở ảnh đầu tiên...`, "loading");
    ...
    async function skipCurrentBatchImage() {
        ...
        state.batch.index += 1;
        setBatchStatus(`Đã bỏ qua ${state.batch.currentFileName || "ảnh hiện tại"}. Đang mở ảnh tiếp theo...`, "loading");
        updateBatchUi();
        await loadCurrentBatchItem();
    }
    ...
    async function loadCurrentBatchItem() {
        ...
        try {
            while (state.batch.active && state.batch.index < state.batch.files.length) {
                ...
                if (faces.length === 0) {
                    setBatchStatus(`Bỏ qua ${file.name} vì không tìm thấy khuôn mặt rõ.`, "error");
                } else {
                    setBatchStatus(`Bỏ qua ${file.name} vì ảnh có nhiều hơn một khuôn mặt.`, "error");
                }
            ...
            console.error(error);
            setBatchStatus("Không đọc được một ảnh trong batch. Bạn có thể chọn folder khác.", "error");
        } finally {
            state.batch.isBusy = false;
            updateBatchUi();
    ...
    function stopBatchSession(options = {}) {
        ...
        if (!silent && hadBatch) {
            ...
            setBatchStatus("Sau khi chọn folder, bạn chỉ cần bấm đúng nhãn hoặc sửa nhãn ở khung feedback.");
        }
    }
    ...
    function resetAnalysis() {
        ...
        state.lastSentAt = 0;
        clearFeedbackState();
        setStatus("Đã làm mới vùng quét. Bạn có thể mở webcam hoặc chọn ảnh khác.", "ready");
    }
    ...
    function loadFileIntoStage(file, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            image.onload = () => {
                ...
                drawImageToCanvas();
                dom.analyzeButton.disabled = !enableAnalyzeButton || !state.faceLandmarker;
                resolve(image);
            };
    ...
    function drawOverlay(landmarks, source) {
        ...
        for (const index of faceOvalIndices) {
            ...
            context.beginPath();
            context.arc(point.x * dom.canvas.width, point.y * dom.canvas.height, 2.2, 0, Math.PI * 2);
    ...
    function spanForBand(points, minY, maxY, startRatio, endRatio) {
        ...
        const bandPoints = points.filter((point) => point.y >= bandStart && point.y <= bandEnd);
    ...
    function classifyFace(metrics) {
        const runtimePrediction = classifyFaceWithRuntimeModel(metrics, state.runtimeModel);
    ...
    function classifyFaceWithRules(metrics) {
        const { lengthToCheekRatio, jawToCheekRatio, foreheadToCheekRatio, foreheadJawDelta } = metrics;
        let shape = "Trái xoan";
        let confidence = 0.72;
        ...
        const isLongLike =
            ...
            (jawToCheekRatio < 0.78 || foreheadToCheekRatio < 0.8 || foreheadJawDelta > 0.14);
        ...
        } else if (lengthToCheekRatio < 1.42 && jawToCheekRatio > 0.76 && foreheadToCheekRatio > 0.76) {
            shape = "Tròn";
            confidence = 0.72 + Math.min(0.16, (1.42 - lengthToCheekRatio) * 0.5);
        } else {
            shape = "Trái xoan";
            confidence = 0.7 + Math.min(0.15, Math.abs(foreheadToCheekRatio - jawToCheekRatio) * 0.2);
        }
    ...
    function classifyFaceWithRuntimeModel(metrics, runtimeModel) {
        if (!runtimeModel || !Array.isArray(runtimeModel.classes) || runtimeModel.classes.length < 2) {
            return null;
        }
        ...
        const means = Array.isArray(runtimeModel.globalMeans) ? runtimeModel.globalMeans : [];
        const stdDevs = Array.isArray(runtimeModel.globalStdDevs) ? runtimeModel.globalStdDevs : [];
        if (means.length !== featureVector.length || stdDevs.length !== featureVector.length) {
            return null;
        }
        ...
        const ranked = runtimeModel.classes
            .map((entry) => {
                if (!Array.isArray(entry.centroid) || entry.centroid.length !== standardized.length) {
                    return null;
                }
        ...
        const confidence = Math.max(0.58, Math.min(0.96, 0.56 + topProbability * 0.24 + (topProbability - secondProbability) * 0.32));
    ...
    function initializeFeedbackButtonLabels() {
        if (dom.feedbackAcceptButton && !dom.feedbackAcceptButton.dataset.defaultLabel) {
            dom.feedbackAcceptButton.dataset.defaultLabel = dom.feedbackAcceptButton.textContent.trim();
        }
    ...
    function updateBatchUi() {
        ...
        if (dom.batchProgress) {
            if (state.batch.active && state.batch.files.length > 0) {
                const current = Math.min(state.batch.index + 1, state.batch.files.length);
                dom.batchProgress.textContent =
                    `Ảnh ${current}/${state.batch.files.length} | Đã lưu ${state.batch.labeledCount} | Bỏ qua ${state.batch.skippedCount}`;
            } else {
        ...
        if (dom.selectBatchFolderButton) {
            dom.selectBatchFolderButton.textContent = state.batch.active ? "Chọn thư mục khác" : "Chọn thư mục ảnh";
        }
    ...
    function captureSnapshotDataUrl() {
        if (state.image) {
            ...
            context.drawImage(state.image, 0, 0, snapshotCanvas.width, snapshotCanvas.height);
            return snapshotCanvas.toDataURL("image/jpeg", 0.92);
        }

        if (state.isCameraRunning && dom.video.readyState >= 2 && dom.video.videoWidth && dom.video.videoHeight) {
            const snapshotCanvas = document.createElement("canvas");
            ...
            context.drawImage(dom.video, 0, 0, snapshotCanvas.width, snapshotCanvas.height);
            context.restore();
    ...
    function renderSuggestions(payload) {
        ...
        for (const suggestion of payload.suggestions) {
            ...
            card.innerHTML = `
                ...
                <div class="result-card__body">
                    ...
                    <a class="btn btn-warning w-100 mt-3" href="${escapeHtml(appRoot.dataset.bookingUrl)}">Đặt lịch kiểu này</a>
                </div>`;
        ...
        if (payload.usedFallbackData) {
            ...
            note.textContent = "Đang dùng bộ kiểu tóc gợi ý dự phòng để trải nghiệm AI không bị gián đoạn.";
    ...
    async function loadRuntimeModel(options = {}) {
        ...
        try {
            ...
            if (response.status === 404) {
                ...
                if (!silent) {
                    setDatasetStatus("Chưa có model train riêng. AI đang dùng thuật toán mặc định.", "success");
                }
                return null;
            }
            ...
            state.runtimeModel = null;
            if (!silent) {
                setDatasetStatus("Không tải được model train, AI sẽ dùng thuật toán mặc định.", "error");
            }
            return null;
        }
    }
    ...
    function normalizeShapeLabelLegacy(value) {
        ...
        if (normalized.includes("tron") || normalized.includes("tròn") || normalized.includes("trã²n") || normalized.includes("round")) {
            return "Tròn";
        }

        if (normalized.includes("vuong") || normalized.includes("vuông") || normalized.includes("vuã´ng") || normalized.includes("square")) {
            return "Vuông";
        }
        ...
        if (normalized.includes("dai") || normalized.includes("dài") || normalized.includes("dã i") || normalized.includes("long") || normalized.includes("oblong")) {
            return "Dài";
        }
    ...
    function normalizeShapeLabel(value) {
        ...
        if (normalized.includes("tron") || normalized.includes("tròn") || normalized.includes("round")) {
            return "Tròn";
        }

        if (normalized.includes("vuong") || normalized.includes("vuông") || normalized.includes("square")) {
            return "Vuông";
        }
        ...
        if (normalized.includes("dai") || normalized.includes("dài") || normalized.includes("long") || normalized.includes("oblong")) {
            return "Dài";
        }
